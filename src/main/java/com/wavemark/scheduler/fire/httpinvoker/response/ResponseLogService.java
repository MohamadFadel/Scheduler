...
public class ResponseLogService {
    ...
    public void logResponse(Task task, HttpResponse httpResponse, long time) throws SchedulerException {
        ...
        Integer taskRunLogId = saveTaskRunLog(task, dbTrigger, httpResponse.getMessage(), httpResponse.getCode(), time);
        updateTaskLog(task, dbTrigger, taskRunLogId);

        if (httpResponse.isSuccess()) {
            log.info("Api call " + StringEscapeUtils.escapeJava(httpResponse.getUrl()) + " was successful.");
    ...
    public void logResponseError(Task task, HttpResponse httpResponse, long time) throws SchedulerException {
        ...
        Trigger dbTrigger = triggerService.getTrigger(task.getTaskName());
        Integer taskLogId = saveTaskRunLog(task, dbTrigger, httpResponse.getMessage(), httpResponse.getCode(), time);

        updateTaskLog(task, dbTrigger, taskLogId);
        taskService.saveTask(task);
    }

    private Integer saveTaskRunLog(Task dbTask, Trigger dbTrigger, String responseMessage, Integer responseCode, long time)
            throws SchedulerException {

        TaskRunLog taskRunLog = taskRunLogService.buildTaskRunLog(dbTask, dbTrigger, responseMessage, responseCode, time);
